...
function reload() {
	...
	for (var i = 1; i < net.layers.length - 1; i++) {
		...
		t +=
			...
			butid +
			'" value="' +
			net.layers[i].layer_type +
			"(" +
			net.layers[i].out_depth +
			")" +
			'" type="submit" onclick="updateLix(' +
			i +
...
function draw() {
	...
	for (var x = 0.0, cx = 0; x <= WIDTH; x += density, cx++) {
		for (var y = 0.0, cy = 0; y <= HEIGHT; y += density, cy++) {
			...
			//ctx.fillStyle =
			//	"rgb(150," + Math.floor(a.w[0] * 105) + 150 + ",150)";

			ctx.fillStyle =
				"rgb(" +
				Math.floor(a.w[0] * 255) +
				"," +
				Math.floor(a.w[1] * 255) +
				", 100," +
				(a.w[0] > a.w[1] ? a.w[0] : a.w[1]) +
				")";
	...
	for (var i = 0; i < N; i++) {
		...
		var a = net.forward(netx, false);
		var xt =
			(visWIDTH * (net.layers[lix].out_act.w[d0] - mmx.minv)) / mmx.dv; // in screen coords
		var yt =
			(visHEIGHT * (net.layers[lix].out_act.w[d1] - mmy.minv)) / mmy.dv; // in screen coords
